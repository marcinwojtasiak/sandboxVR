...
namespace Mirror
{
    ...
    public class CustomNetworkManager : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            if (!NetworkClient.isConnected && !NetworkServer.active)
            {
                if (!NetworkClient.active)
                {
                    ...
                    if (GUILayout.Button("IP Janusza"))
                    {
                        manager.networkAddress = "25.61.86.220";
                    }
                    if (GUILayout.Button("IP Mąrcina"))
                    {
                        manager.networkAddress = "25.61.82.211";
                    }
        ...
        {
            if (collider.name == "Controller (right)")
            {
            ...
            if (collider.name == "Controller (left)")
            {
                //GetComponent<BoxCollider>().enabled = false;
                //GetComponent<MeshRenderer>().enabled = false;
