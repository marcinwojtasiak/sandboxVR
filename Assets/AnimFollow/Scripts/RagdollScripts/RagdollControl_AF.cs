...
namespace AnimFollow
{
	...
	//	 This kind of a state machine that takes the character through the states: colliding, falling, matching the masters pose and getting back up
		...
		// moj shit
		[Range(0f, 10f)] public float getupTorque = 1f;               // The torque when getting up
		[Range(0f, 10f)] public float getupForce = 2f;
		[Range(0f, 50000f)] public float getupJointTorque = 1000f;
		//----
				...
				if (gettingUp)
				{
					...
					if (orientated)
					{
						if (animFollow.maxTorque < wakeUpStrength) // Ease the ragdoll to the master pose. WakeUpStrength limit should be set so that the radoll just has reached the master pose
						{
							...
							animFollow.maxTorque = Mathf.Lerp(animFollow.maxTorque, getupTorque, getupLerp1 * Time.fixedDeltaTime);
							animFollow.maxForce = Mathf.Lerp(animFollow.maxForce, getupForce, getupLerp1 * Time.fixedDeltaTime);
							animFollow.maxJointTorque = Mathf.Lerp(animFollow.maxJointTorque, getupJointTorque, getupLerp1 * Time.fixedDeltaTime); // moje zmiany
						...
						else // Lerp the ragdoll to contact strength during get up
						{
							...
							animFollow.maxTorque = Mathf.Lerp(animFollow.maxTorque, getupTorque, getupLerp2 * Time.fixedDeltaTime);
							animFollow.maxForce = Mathf.Lerp(animFollow.maxForce, getupForce, getupLerp2 * Time.fixedDeltaTime);
							animFollow.maxJointTorque = Mathf.Lerp(animFollow.maxJointTorque, getupJointTorque, getupLerp2 * Time.fixedDeltaTime); // moje zmiany
