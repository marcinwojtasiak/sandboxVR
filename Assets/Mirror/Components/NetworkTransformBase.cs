...
namespace Mirror
{
    public abstract class NetworkTransformBase : NetworkBehaviour
    {
        ...
        // is there attempt to get the authority?
        bool localAuthPending = false;

        // time since trying to get the authority
        float timeSinceAuthPing = 0;
        ...
        void Update()
        {
            ...
            if (isClient)
            {
                timeSinceAuthPing += Time.deltaTime;
                if(timeSinceAuthPing > 0.5f)
                {
                    localAuthPending = false;
                }
                ...
                if (!hasAuthority && !localAuthPending)
                {
                    // TODO: IF TRYING TO HOLD LOCALLY / BUMP  //Done?
        ...
        public void GetTemporaryAuth()
        {
            localAuthPending = true;
            timeSinceAuthPing = 0;
        }
    }
}
